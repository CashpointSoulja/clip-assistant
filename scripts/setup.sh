...
if [[ ! -f "$RUNTIME/ggml-base.en.bin" ]]; then
  (cd "$SRC" && bash models/download-ggml-model.sh base.en)
  cp "$SRC/models/ggml-base.en.bin" "$RUNTIME/ggml-base.en.bin"
fi
printf 'FFMPEG_BIN=%s\nFFPROBE_BIN=%s\nWHISPER_BIN=%s\nWHISPER_MODEL=%s\n' "$(command -v ffmpeg)" "$(command -v ffprobe)" "$RUNTIME/whisper-cli" "$RUNTIME/ggml-base.en.bin"
