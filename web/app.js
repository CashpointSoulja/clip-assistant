...
function rangeLabel(clip) {
  const ranges = Array.isArray(clip?.ranges) ? clip.ranges : [];
  return ranges.length === 1 ? `${fmt(ranges[0].start)}–${fmt(ranges[0].end)}` : `${ranges.length} ranges`;
}
function initIntro() {
...
function renderClips(clips) { text($('clip-count'), clips.length); const root = $('clips'); root.replaceChildren(); clips.forEach((clip, index) => { const card = document.createElement('article'); const titleText = clipDescription(clip, index); const riskText = clip.risks ? (Array.isArray(clip.risks) ? clip.risks.join('; ') : clip.risks) : ''; card.className = `clip-card ${state.clip?.id === clip.id ? 'selected' : ''}`; card.tabIndex = 0; card.setAttribute('role', 'button'); card.setAttribute('aria-label', `Review ${titleText}, ${rangeLabel(clip)}. ${clip.reason || 'Needs review'}${riskText ? `. ${riskText}` : ''}`); card.setAttribute('aria-controls', 'clip-editor'); const title = document.createElement('h4'); title.title = titleText; text(title, titleText); const meta = document.createElement('div'); meta.className = 'clip-meta'; const score = document.createElement('span'); score.className = 'score'; text(score, clip.score == null ? 'Local · no ranking' : `Score ${clip.score}`); const duration = document.createElement('span'); text(duration, rangeLabel(clip)); meta.append(score, duration); const reason = document.createElement('p'); reason.className = 'clip-reason clip-detail'; text(reason, clip.reason || 'No reason supplied.'); card.append(title, meta, reason); const values = clip.criteria || {}; const hasCriteria = ['hook', 'specificity', 'payoff', 'audience_fit', 'coherence'].some(key => values[key] != null); if (hasCriteria) { const criteria = document.createElement('small'); criteria.className = 'clip-reason clip-criteria clip-detail'; const labels = ['hook', 'specificity', 'payoff', 'audience_fit', 'coherence']; text(criteria, `Criteria (0–4): ${labels.map(key => `${key}: ${values[key] ?? '—'}`).join(' · ')}`); card.append(criteria); } if (riskText) { const risk = document.createElement('small'); risk.className = 'risk clip-detail'; text(risk, `Review: ${riskText}`); card.append(risk); } const choose = () => openEditor(clip); card.addEventListener('click', choose); card.addEventListener('keydown', e => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); choose(); } }); root.append(card); }); const sequence = $('play-sequence'); const sequenceLabel = $('sequence-label'); if (sequence) { const ready = Boolean(state.clip && state.ranges.length); sequence.disabled = !ready; sequence.setAttribute('aria-disabled', String(!ready)); sequence.title = ready ? 'Play the selected ranges' : 'Select a candidate clip first'; if (sequenceLabel) { sequenceLabel.classList.toggle('sr-only', !ready); text(sequenceLabel, ready ? `${state.ranges.length} range${state.ranges.length === 1 ? '' : 's'}` : 'Select a clip'); } } }
function previewClip(clip) { const video = $('video'); const start = Number(clip?.ranges?.[0]?.start); if (!video || !Number.isFinite(start)) return; if (sequenceStop) { video.removeEventListener('timeupdate', sequenceStop); sequenceStop = null; } const seek = () => { video.currentTime = start; video.play().catch(() => {}); }; if (video.readyState >= 1) seek(); else video.addEventListener('loadedmetadata', seek, { once: true }); }
function openEditor(clip) { state.clip = clip; state.ranges = (clip.ranges || []).map(r => ({ start: Number(r.start), end: Number(r.end) })); const editor = $('clip-editor'); const clips = $('clips'); if (editor && clips?.parentElement) clips.parentElement.insertBefore(editor, clips); editor.hidden = false; text($('editor-title'), clipDescription(clip)); text($('editor-reason'), (clip.reason || '').replace(/^LOCAL heuristic suggestion; editor must assess /i, 'Review ') || 'Review ranges'); renderRanges(); renderClips(state.job.clips || []); previewClip(clip); editor.scrollIntoView({ block: 'center', behavior: window.matchMedia?.('(prefers-reduced-motion: reduce)').matches ? 'auto' : 'smooth' }); const heading = $('editor-title'); heading.tabIndex = -1; heading.focus({ preventScroll: true }); }
...
function initWorkflowAffordances() {
  ...
  const sync = () => {
    ...
    if (state.job && stage) { const candidateCount = Number(clips) || 0; const exportLabel = `${exports} export${exports === 1 ? '' : 's'}`; const candidateLabel = `${candidateCount} candidate${candidateCount === 1 ? '' : 's'}`; const value = ['REVIEW READY', 'EXPORT READY'].includes(stage) ? `${candidateLabel} · ${exportLabel}` : `${stage} · ${candidateLabel}`; if (summary.textContent !== value) text(summary, value); }
